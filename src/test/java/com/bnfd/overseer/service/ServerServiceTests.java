...
//
//            Server results = serverService.getServerById(testId, false);
//
//            Assertions.assertTrue(ObjectUtils.isNotEmpty(results));
//
//            Mockito.verify(serverService, Mockito.times(1)).getServerById(Mockito.anyString(), Mockito.anyBoolean());
...
//
//            Assertions.assertThrows(OverseerNotFoundException.class, () -> serverService.getServerById(UUID.randomUUID().toString(), false));
//
//            Mockito.verify(serverService, Mockito.times(1)).getServerById(Mockito.anyString(), Mockito.anyBoolean());
//            Mockito.verify(serverRepository, Mockito.times(1)).findById(Mockito.anyString());
//            Mockito.verify(overseerMapper, Mockito.times(0)).map(Mockito.any(ServerEntity.class), Mockito.eq(Server.class));
...
//
//            List<Server> results = serverService.getAllServers(false);
//
//            Assertions.assertFalse(CollectionUtils.isEmpty(results));
//
//            Mockito.verify(serverService, Mockito.times(1)).getAllServers(Mockito.anyBoolean());
...
//
//            Assertions.assertThrows(OverseerNoContentException.class, () -> serverService.getAllServers(false));
//
//            Mockito.verify(serverService, Mockito.times(1)).getAllServers(Mockito.anyBoolean());
//            Mockito.verify(serverRepository, Mockito.times(1)).findAll();
...
//        MockedStatic<ValidationUtils> mockedValidationUtils;
//        MockedStatic<DefaultSettings> mockedDefaultSettings;
//
//        @BeforeEach
//        public void preTestSetup() {
//            Map<String, String> defaultSettings = new HashMap<String, String>();
//            defaultSettings.put("active", "true");
//
//            mockedValidationUtils = Mockito.mockStatic(ValidationUtils.class);
//            mockedDefaultSettings = Mockito.mockStatic(DefaultSettings.class);
//
//            mockedValidationUtils.when(() -> ValidationUtils.convertStringToBoolean(Mockito.any())).thenCallRealMethod();
//            mockedValidationUtils.when(DefaultSettings::getSettings).thenReturn(defaultSettings);
//        }
//
//        @AfterEach
//        public void posTestTeardown() {
//            mockedValidationUtils.close();
//            mockedValidationUtils = null;
//            mockedDefaultSettings.close();
//            mockedDefaultSettings = null;
//        }
//
//        @Test
...
//
//            Assertions.assertTrue(results.getSettings().stream().anyMatch(setting -> setting.getName().equalsIgnoreCase(Constants.ACTIVE_SETTING)));
//
//            Mockito.verify(serverService, Mockito.times(1)).configureNewServer(Mockito.any(ServerEntity.class));
//        }
//    }
